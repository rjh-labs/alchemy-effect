...
export class LLM extends Context.Tag("LLM")<LLM, LLMService>() {}
...
export interface LLMService {
  ...
  ) => Stream.Stream<StreamTextPart, LLMError, Tools["Req"]>;
}
        ...
        return Stream.fromEffect(
          Effect.gen(function* () {
            ...
            for (const t of input.tools) {
              const decode = S.decode(t.input);
              tools[t.alias?.(modelId) ?? t.id] = tool({
                inputSchema: jsonSchema(JSONSchema.make(t.input)),
                description: "TODO",
                execute: (params) =>
                  Effect.runPromise(
                    decode(params).pipe(
                      Effect.flatMap(t.execute),
